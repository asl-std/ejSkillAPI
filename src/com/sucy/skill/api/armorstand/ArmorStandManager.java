...
public class ArmorStandManager {
	...
	/**
	 * Registers the armor stand repeated task, and searches for rogue armor to
	 * remove them
	 */
	public static void init() {
		MainThread.register(new ArmorStandTask());
		Bukkit.getWorlds().forEach(world -> world.getEntitiesByClass(ArmorStand.class).forEach(
				as -> {
					if (SkillAPI.getMeta(as, MechanicListener.ARMOR_STAND) != null)
						as.remove();
				}));
	}
	...
	public static void tick() {
		final Iterator<ArmorStandData> iterator = instances.values().iterator();
		while (iterator.hasNext()) {
			final ArmorStandData data = iterator.next();
			if (data.isValid()) {
				data.tick();
			} else {
				data.remove();
