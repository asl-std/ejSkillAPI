...
/**
 * SkillAPI © 2018 com.sucy.skill.cmd.CmdChangeClass
 */
		...
		if (args.length >= 3) {
			...
			if (data == null) {
				cmd.sendMessage(sender, INVALID_GROUP, "{player} does not have a {group}",
						Filter.PLAYER.setReplacement(player.getName()), RPGFilter.CLASS.setReplacement(groupName));
				return;
			}
			...
			cmd.sendMessage(sender, SUCCESS, "You have changed {player} from a {name} to a {group}",
					Filter.PLAYER.setReplacement(player.getName()), RPGFilter.CLASS.setReplacement(className),
					RPGFilter.NAME.setReplacement(original));

			if (sender != player) {
				cmd.sendMessage(player, NOTIFICATION, "You have changed from a {name} to a {group}", RPGFilter.CLASS,
						RPGFilter.NAME);
			}
		} else {
